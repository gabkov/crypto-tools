...
pub fn run(abi_path: &str, function: &str, args: &[String]) -> Result<String, String> {
...
fn encode_call(abi_src: &str, fn_name: &str, args: &[String]) -> Result<String, String> {
    let abi: JsonAbi =
    ...
    for (input, arg) in func.inputs.iter().zip(args) {
        let ty: DynSolType = input
            .resolve()
            .map_err(|e| format!("resolving type '{}': {e}", input.ty))?;
        let value = ty
            .coerce_str(arg)
            .map_err(|e| format!("parsing '{arg}' as {}: {e}", input.ty))?;
        values.push(value);
    }
    ...
    let encoded = func
        ...
        .map_err(|e| format!("encoding arguments: {e}"))?;
...
mod tests {
    ...
    fn errors_on_unknown_function() {
        ...
        assert!(err.contains("no function named 'mint'"));
    }
    ...
    fn errors_on_wrong_arity() {
        ...
        assert!(err.contains("expects 2 argument(s), got 1"));
    }
}
