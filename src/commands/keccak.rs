...
impl<'a> Keccak<'a> {
    pub fn new(input: &'a str, as_hex: bool) -> Self {
        Keccak { input, as_hex }
    }
}

impl<'a> Command for Keccak<'a> {
...
mod tests {
    ...
    #[test]
    fn hashes_utf8_text() {
        let keccak = Keccak::new("hello", false);
        assert_eq!(
            keccak.run().unwrap(),
    ...
    fn hex_mode_differs_from_text_mode() {
        ...
        let keccak1 = Keccak::new("0x68656c6c6f", true);
        let keccak2 = Keccak::new("hello", false);
        assert_eq!(keccak1.run().unwrap(), keccak2.run().unwrap());
    }

    #[test]
    fn errors_on_bad_hex() {
        let keccak = Keccak::new("0xzz", true);
